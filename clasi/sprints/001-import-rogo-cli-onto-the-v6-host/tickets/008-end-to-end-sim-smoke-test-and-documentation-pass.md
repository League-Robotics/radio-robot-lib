...
- [ ] One scripted end-to-end test runs the full command surface against
      `tools/sim` in sequence and passes.
- [ ] `docs/design/usecases.md` UC-014/UC-015/UC-016 no longer read
      "planned"; each cites the actual module implementing it.
- [ ] `docs/design/overview.md`'s status paragraph reflects Rogo's import
      as done, not upcoming.
- [ ] A short doc (README or module docstring) in `src/host/rogo/` lists
...
- [ ] Full test suite (`uv run pytest`) passes — this is the last ticket
