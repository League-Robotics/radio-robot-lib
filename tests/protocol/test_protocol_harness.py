...
def _load_shim(tmp_path):
    ...
    lib.phSetRunEntry.argtypes = [
        ctypes.c_void_p, ctypes.c_uint, ctypes.c_char_p, ctypes.c_char_p]
    lib.phSetRunEntry.restype = None
    lib.phSetRunEntryCount.argtypes = [ctypes.c_void_p, ctypes.c_uint]
    lib.phSetRunEntryCount.restype = None
...
class _GoldenVectorRunner:
    ...
    def apply_setup(self, key, tokens):
        ...
        if key == "identity":
            ...
            # runentry <index> <name> <signature|-> -- '-' declares no
            # signature, since the fixture's own tokenizer cannot carry
            # an empty token any more than the wire grammar can.
            index, name, signature = tokens
            lib.phSetRunEntry(
                handle, int(index), self._keep(name),
                self._keep("" if signature == "-" else signature))
        elif key == "runentrycount":
            lib.phSetRunEntryCount(handle, int(tokens[0]))
        elif key == "lastdone":
...
# ---- FUNCS: enumerating the RUN registry (docs/design/protocol.md
# §6.5). The mock's registry is settable and defaults to EMPTY, so a
# test that does not opt in is exercising the empty-allowlist path. ----


def _register(lib, handle, *entries):
    """Install `entries` -- (name, signature) pairs -- as the mock's RUN
    registry. Encoded bytes are returned so the caller can keep them
    alive: MockAdapter stores BORROWED pointers, exactly like
    fieldNames/runResultText."""
    keepalive = []
    for index, (name, signature) in enumerate(entries):
        name_b, sig_b = name.encode("ascii"), signature.encode("ascii")
        keepalive += [name_b, sig_b]
        lib.phSetRunEntry(handle, index, name_b, sig_b)
    lib.phSetRunEntryCount(handle, len(entries))
    return keepalive


def test_funcs_on_an_empty_registry_is_the_ack_alone(tmp_path):
    """An empty allowlist answers with NO funcs lines -- the wire-visible
    shape of "nothing is callable here", not an error (§6.5). This is
    what DiffDriveAdapter itself does."""
    lib = _load_shim(tmp_path)
    handle = lib.phCreate()
    try:
        _feed(lib, handle, "FUNCS #1\n")
        assert _sink_lines(lib, handle) == [_ack(1)]
    finally:
        lib.phDestroy(handle)


def test_funcs_lists_every_entry_in_registration_order(tmp_path):
    lib = _load_shim(tmp_path)
    handle = lib.phCreate()
    try:
        keep = _register(lib, handle, ("add", "int,int->int"),
                          ("blink", "int->void"), ("ping", "->int"))
        _feed(lib, handle, "FUNCS #1\n")
        assert _sink_lines(lib, handle) == [
            _ack(1),
        ...
        assert keep  # borrowed pointers stayed alive for the whole call
    finally:
        lib.phDestroy(handle)


def test_funcs_omits_the_field_entirely_when_no_signature_is_declared(
        tmp_path):
    """An empty signature must not leave a dangling separator space --
    the grammar has no empty token (§6.5, same rule sendDebug("")
    follows)."""
    lib = _load_shim(tmp_path)
    handle = lib.phCreate()
    try:
        keep = _register(lib, handle, ("add", "int,int->int"), ("blink", ""),
                          ("stop", ""))
        _feed(lib, handle, "FUNCS #1\n")
        assert _sink_lines(lib, handle) == [
            _ack(1),
...
def test_funcs_appears_in_help(tmp_path):
    """HELP is generated by walking kCommandTable, so FUNCS joining the
    table is the only thing that can put it in HELP's listing."""
    lib = _load_shim(tmp_path)
    handle = lib.phCreate()
    try:
        _feed(lib, handle, "HELP #1\n")
        help_line = [l for l in _sink_lines(lib, handle)
                     if l.startswith("help ")]
        assert len(help_line) == 1
        assert help_line[0].split() == [
            "help", "HELLO", "PING", "ID", "VER", "STATUS", "HELP", "GET",
            "SET", "TLM", "WHEELS_X", "WHEELS_V", "MOVE_X", "MOVE_V",
            "GO_TO_R", "GO_TO_W", "STOP", "ESTOP", "FUNCS", "RUN"]
    finally:
        lib.phDestroy(handle)


def test_funcs_with_a_stray_field_is_a_decode_failure(tmp_path):
    """FUNCS shares decodeNoFields with ID/VER/STATUS/HELP, so a stray
    data field nacks and does NOT advance the sequence (§8.9)."""
    lib = _load_shim(tmp_path)
    handle = lib.phCreate()
    try:
        _register(lib, handle, ("add", "int,int->int"))
        _feed(lib, handle, "FUNCS x #1\n")
        assert _sink_lines(lib, handle) == [_nack(1), "err 2 #1"]
        assert lib.phMalformedCount(handle) == 1
    finally:
        lib.phDestroy(handle)
...
def test_funcs_without_an_id_gets_no_reply_at_all(tmp_path):
    """FUNCS is SEQUENCED (§6.5) -- unlike HELP/ID/VER/STATUS it is not
    in the unsequenced exemption set, so a missing id is malformed and
    answered with silence, not with a listing."""
    lib = _load_shim(tmp_path)
    handle = lib.phCreate()
    try:
        _register(lib, handle, ("add", "int,int->int"))
        _feed(lib, handle, "FUNCS\n")
        assert _sink_lines(lib, handle) == []
        assert lib.phMalformedCount(handle) == 1
    finally:
        lib.phDestroy(handle)
...
def test_funcs_id_zero_is_a_stale_retransmit_and_lists_nothing(tmp_path):
    lib = _load_shim(tmp_path)
    handle = lib.phCreate()
    try:
        _register(lib, handle, ("add", "int,int->int"))
        _feed(lib, handle, "FUNCS #0\n")
        lines = _sink_lines(lib, handle)
        assert not [l for l in lines if l.startswith("funcs")]
        assert lines == [_ack(0)]
    finally:
        lib.phDestroy(handle)


def test_funcs_names_are_sanitized_and_cannot_forge_a_second_line(tmp_path):
    """Registry strings are adapter-supplied free-form text, sanitized
    exactly like debug's text and RUN's returned value (§6.5)."""
    lib = _load_shim(tmp_path)
    handle = lib.phCreate()
    try:
        keep = _register(lib, handle, ("ev\nil", "a\r\nb"))
        _feed(lib, handle, "FUNCS #1\n")
        assert _sink_lines(lib, handle) == [_ack(1), "funcs evil ab"]
        assert keep
    finally:
        lib.phDestroy(handle)


def test_funcs_skips_an_entry_whose_name_sanitizes_away(tmp_path):
    """A nameless entry is not addressable by RUN, so it is skipped
    rather than emitted as a bare `funcs` line."""
    lib = _load_shim(tmp_path)
    handle = lib.phCreate()
    try:
        keep = _register(lib, handle, ("add", ""), ("\n\r", "int->int"),
                          ("blink", ""))
        _feed(lib, handle, "FUNCS #1\n")
        assert _sink_lines(lib, handle) == [
            _ack(1), "funcs add", "funcs blink"]
        assert keep
    finally:
        lib.phDestroy(handle)


def test_funcs_truncates_an_overlong_entry_and_still_terminates_it(tmp_path):
    """snprintf truncates; the '\\n' is appended off the RESULTING length,
    so an adapter that overruns the 240-byte cap loses characters but can
    never merge two wire lines into one."""
    lib = _load_shim(tmp_path)
    handle = lib.phCreate()
    try:
        keep = _register(lib, handle, ("x" * 300, "y" * 300), ("after", ""))
        _feed(lib, handle, "FUNCS #1\n")
        lines = _sink_lines(lib, handle)
        assert lines[0] == _ack(1)
        assert lines[1].startswith("funcs xxx")
        # 240 bytes of wire content INCLUDING the '\n' _sink_lines split
        # off -- so 239 characters survive here.
        assert len(lines[1]) == 239
        assert lines[2] == "funcs after", "the next entry is its own line"
        assert keep
    finally:
        lib.phDestroy(handle)
